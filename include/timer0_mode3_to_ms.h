 ...
 * @ingroup timer0_mode3_to_ms
 */
#define timer0_mode3_Hz_to_ticks(frequency) (timer_Hz_to_ticks(frequency, get_timer0_clock_divider(), get_frequency_divider_scale()))
...
/**
 * @brief Start timer0 in mode 3 with ms duration
 * 
 * @param ms uint16_t timer overflow duration
 * 
 * @ingroup timer0_mode3_to_ms
 */
#define timer0_mode3_start_ms(ms)                       \
do                                                      \
{                                                       \
...
/**
 * @brief Start timer0 in mode 3 with Hz frequency
 * 
 * @param freq uint32_t timer overflow frequency in Hz
 * 
 * @ingroup timer0_mode3_to_ms
 */
#define timer0_mode3_start_Hz(freq)                     \
do                                                      \
{                                                       \
    uint16_t ticks = timer0_mode3_Hz_to_ticks(freq);    \
    timer0_mode3_start(ticks);                          \
} while (0);

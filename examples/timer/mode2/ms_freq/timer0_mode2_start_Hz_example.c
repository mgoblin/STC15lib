/**
 * @file timer0_mode2_start_Hz_example.c
 * @brief Example demonstrating Timer0 Mode 2 operation with frequency-based timing
 * 
 * This example shows how to use Timer0 in Mode 2 (8-bit timer mode) with
 * frequency-based configuration. It demonstrates the timer0_mode2_start_Hz()
 ...
 * and starts the timer with a 20000Hz overflow frequency (since the example uses
 * TIMER0_FREQ_HZ >> 1 = 20000Hz). Each time the timer overflows, an interrupt
 ...
 * (20000Hz / 10000 = 2Hz LED toggle = 1 blink per second).
 * 
 * Key components demonstrated:
 * - Timer0 initialization in Mode 2 with 12T timing
 * - Starting Timer0 with frequency in Hertz using timer0_mode2_start_Hz()
 ...
 * @see timer0_mode2_to_ms.h - Header containing frequency-based timer functions
 * @see timer0_mode2._Hz_to_ticks - Function for manual Hz to ticks conversion
 * 
 */
...
/**
 * @brief Main function for Timer0 Mode 2 frequency-based timing example
 * 
 * This function initializes the timer and configures Timer0 to overflow
 * at the specified frequency (TIMER0_FREQ_HZ << 1 = 20000Hz). The timer operates
 ...
 * 3. Timer overflow interrupts toggle the LED at 1Hz (20000Hz / 10000)
 ...
 *       doubles the base frequency to 20000Hz for a 1Hz LED blink rate
 */
void main(void)
{
    // Initialize Timer0 in Mode 2 (8-bit timer) with 12T timing
    ...
    timer0_mode2_start_Hz(TIMER0_FREQ_HZ << 1);

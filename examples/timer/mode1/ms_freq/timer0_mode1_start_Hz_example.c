/**
 * @file timer0_mode1_start_Hz_example.c
 * @brief Example demonstrating Timer0 Mode 1 operation with frequency-based timing
 * 
 * This example shows how to use Timer0 in Mode 1 (16-bit timer mode) with
 * frequency-based configuration. It demonstrates the timer0_mode1_start_Hz()
 ...
 * - Timer0 initialization in Mode 1 with 12T timing
 * - Starting Timer0 with frequency in Hertz using timer0_mode1_start_Hz()
 ...
 *       overflow frequency.
 * 
 * @see timer0_mode1.h - Header for Timer0 Mode 1 functions
 * @see timer0_mode1_to_ms.h - Header containing frequency-based timer functions
 * @see timer0_mode1._Hz_to_ticks - Function for manual Hz to ticks conversion
 * 
 */
...
/**
 * @brief Main function for Timer0 Mode 1 frequency-based timing example
 * 
 ...
 * 
 * Operation flow:
 * 1. Initialize Timer0 in Mode 1 with 12T timing (one timer count per 12 clock cycles)
...
void main(void)
{
    // Initialize Timer0 in Mode 1 (16-bit timer) with 12T timing
    ...
    timer0_mode1_start_Hz(TIMER0_FREQ_HZ << 1);

/**
 * @file timer0_mode1_start_ms_example.c
 * @brief Example demonstrating Timer0 Mode 1 operation with millisecond-based timing
 * 
 * This example shows how to use Timer0 in Mode 1 (16-bit timer mode) with
 * millisecond-based configuration. It demonstrates the timer0_mode1_start_ms()
 ...
 * - Starting Timer0 with millisecond duration using timer0_mode1_start_ms()
 ...
 * 
 * @see timer0_mode1.h - Header for Timer0 Mode 0 functions
 * @see timer0_mode1_to_ms.h - Header containing millisecond-based timer functions
 * 
 */
...
/**
 * @brief Main function for Timer0 Mode 1 millisecond timing example
 * 
 ...
 * 
 * Operation flow:
 * 1. Configure Timer0 in Mode 1 with 12T timing (one timer count per 12 clock cycles)
...
void main(void)
{
    ...
    timer0_mode1_start_ms(TIMER_DURATION_MS);

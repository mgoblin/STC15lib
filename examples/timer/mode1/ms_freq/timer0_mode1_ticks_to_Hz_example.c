/**
 * @file timer0_mode1_ticks_to_Hz_example.c
 ...
 * corresponding frequency in Hz for Timer0 in Mode 1 (16-bit timer mode).
 * 
 ...
 * - Converting timer ticks to frequency using timer0_mode1_ticks_to_Hz()
...
#define TICKS 0xFFFFU // uint16 modify to change timer frequency
...
void main()
{    
    // Initialize Timer0 in Mode 1 (13-bit timer) with 1T timing
    ...
    // The function timer0_mode1_ticks_to_Hz() converts ticks to frequency in Hz
    const uint32_t timer_frequency = timer0_mode1_ticks_to_Hz(TICKS);

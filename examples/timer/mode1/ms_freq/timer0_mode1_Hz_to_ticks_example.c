/**
 * @file timer0_mode1_Hz_to_ticks_example.c
 ...
 * corresponding number of timer ticks for Timer0 in Mode 1 (16-bit timer mode).
 * 
 ...
 * timer0_mode1_Hz_to_ticks() function. It then starts the timer and toggles an LED
 * on each timer overflow via interrupt.
 * 
 * Key components demonstrated:
 * - Setting MCU clock divider scale
 * - Timer0 initialization in Mode 1 with 1T timing
 * - Converting frequency to timer ticks using timer0_mode1_Hz_to_ticks()
 ...
 *       With the default 11.0592 MHz clock, divider scale 7, and 1T timer mode,
 *       a 1 Hz overflow frequency corresponds to approximately 86399 ticks.
 * 
 * @see timer0_mode1_to_ms.h - Header containing Hz to ticks conversion functions
...
void main(void)
{
    ...
    // Initialize Timer0 in Mode 1 with 1T timing
    timer0_mode1_1T_init();
    // Enable Timer0 interrupt
    enable_timer0_interrupt();

    // Calculate the timer ticks corresponding to the desired frequency
    const uint16_t ticks = timer0_mode1_Hz_to_ticks(FREQ_HZ);

/**
 * @file timer0_mode1_delay_ms_example.c
 * @brief Example demonstrating Timer0 Mode 1 blocking delay with millisecond precision
 * 
 * This example shows how to use Timer0 in Mode 0 (16-bit timer mode) with
 * millisecond-based blocking delays. It demonstrates the timer0_mode1_delay_ms()
 ...
 * - Timer0 initialization in Mode 1 with 1T timing (fastest mode)
 * - Using timer0_mode1_delay_ms() for blocking millisecond delays
 ...
 * 
 * @note The timer0_mode1_delay_ms() function blocks program execution until
...
/**
 * @brief Main function for Timer0 Mode 1 blocking delay example
 * 
 ...
 * 
 * Operation flow:
 * 1. Initialize Timer0 in Mode 1 with 1T timing (one timer count per clock cycle)
...
void main(void)
{
    // Initialize Timer0 in Mode 1 (16-bit timer) with 1T timing
    ...
    while (1) {
        ...
        {
            timer0_mode1_delay_ms(DELAY_MS);
        }

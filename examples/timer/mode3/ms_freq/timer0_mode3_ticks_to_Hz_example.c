/**
 * @file timer0_mode3_ticks_to_Hz_example.c
 ...
 * corresponding frequency in Hz for Timer0 in Mode 3 (split mode).
 * 
 * The example configures Timer0 to run in 1T mode (one clock cycle per count)
 * It demonstrates how to calculate the timer frequency from a given tick value
 * and outputs this information over UART.
 * 
 * Key components demonstrated:
 * - Timer0 initialization in Mode 3 with 1T timing
 * - Converting timer ticks to frequency using timer0_mode3_ticks_to_Hz()
...
{    
    ...
    // The function timer0_mode3_ticks_to_Hz() converts ticks to frequency in Hz
    const uint32_t timer_frequency = timer0_mode3_ticks_to_Hz(TICKS);

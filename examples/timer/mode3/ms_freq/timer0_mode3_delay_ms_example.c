/**
 * @file timer0_mode3_delay_ms_example.c
 * @brief Example demonstrating Timer0 Mode 3 blocking delay with millisecond precision
 * 
 * This example shows how to use Timer0 in Mode 3 (16-bit timer mode) with
 * millisecond-based blocking delays. It demonstrates the timer0_mode3_delay_ms()
 ...
 * - Timer0 initialization in Mode 3 with 1T timing (fastest mode)
 * - Using timer0_mode3_delay_ms() for blocking millisecond delays
 ...
 * 
 * @note The timer0_mode3_delay_ms() function blocks program execution until
 ...
 * @see timer0_mode3_to_ms.h - Header containing millisecond-based timer functions
 * 
 */
...
/**
 * @brief Main function for Timer0 Mode 3 blocking delay example
 * 
 ...
 * 
 * Operation flow:
 * 1. Initialize Timer0 in Mode 3 with 1T timing (one timer count per clock cycle)
...
void main(void)
{
    // Initialize Timer0 in Mode 3 (16-bit timer) with 1T timing
    ...
    while (1) {
        ...
        {
            timer0_mode3_delay_ms(DELAY_MS);
        }

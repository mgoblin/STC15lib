/** 
 * @file timer0_mode3_ticks_to_ms_example.c
 * Example: Convert timer ticks to milliseconds for Timer0 in Mode 3 (16-bit timer)
 *
 * This example demonstrates how to calculate the duration in milliseconds
 * corresponding to a given number of timer ticks for Timer0 operating in Mode 3
 ...
 * 3. Configure Timer0 in Mode 3 with 1T clock, disable its interrupt, and
...
void main()
{
   ...
   // Configure Timer0 in Mode 3 with 1T clock (no prescaler).
   ...
   // This routine must be called after timer0_mode3_1T_init()
   uint32_t ms = timer0_mode3_ticks_to_ms(TICKS);

/**
 * @file timer2_mode0_Hz_to_ticks_example.c
 * @brief Example demonstrating conversion from frequency (Hz) to timer2 ticks
 * 
 * This example shows how to convert a desired frequency in Hz to the
 * corresponding number of timer ticks for Timer2 in Mode 0 (16-bit timer mode).
 * 
 ...
 * timer2_mode0_Hz_to_ticks() function. It then starts the timer and toggles an LED
 * on each timer overflow via interrupt.
 * 
 * Key components demonstrated:
 * - Setting MCU clock divider scale
 * - Timer2 initialization in Mode 0 with 12T timing
 * - Converting frequency to timer ticks using timer2_mode0_Hz_to_ticks()
...
void main(void)
{
    ...
    // Initialize Timer2 in Mode 0 with 12T timing
    timer2_mode0_12T_init();
    // Enable Timer2 interrupt
    enable_timer2_interrupt();

    // Calculate the timer ticks corresponding to the desired frequency
    const uint16_t ticks = timer2_mode0_Hz_to_ticks(FREQ_HZ);
    // Start Timer2 with the calculated ticks
